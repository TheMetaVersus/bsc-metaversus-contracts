...
describe("TokenERC1155:", () => {
    beforeEach(async () => {
        ...
        tokenERC1155 = await upgrades.deployProxy(TokenERC1155, [treasury.address, 250, admin.address]);
    ...
    describe("Deployment:", async () => {
        it("Should revert when invalid admin contract address", async () => {
            await expect(
                upgrades.deployProxy(TokenERC1155, [treasury.address, 250, constants.ZERO_ADDRESS])
            ).to.revertedWith("Invalid Admin contract");
            await expect(upgrades.deployProxy(TokenERC1155, [treasury.address, 250, user1.address])).to.revertedWith(
                "Invalid Admin contract"
            );
            await expect(upgrades.deployProxy(TokenERC1155, [treasury.address, 250, treasury.address])).to.revertedWith(
                "Invalid Admin contract"
            );
        });
    ...
    describe("setTreasury function:", async () => {
        it("should revert when caller is not owner: ", async () => {
            await expect(tokenERC1155.connect(user1).setTreasury(user2.address)).to.be.revertedWith(
                "Adminable: caller is not an owner or admin"
            );
        });
        it("should revert when address equal to zero address: ", async () => {
            await expect(tokenERC1155.setTreasury(constants.ZERO_ADDRESS)).to.be.revertedWith(
                "ERROR: invalid address !"
            );
        });
        it("should set treasury success: ", async () => {
            await tokenERC1155.setTreasury(treasury.address);
            expect(await tokenERC1155.treasury()).to.equal(treasury.address);

            await tokenERC1155.setTreasury(user1.address);
            expect(await tokenERC1155.treasury()).to.equal(user1.address);
    ...
    describe("mint function:", async () => {
        ...
        it("should mint success: ", async () => {
            ...
            await token.connect(owner).approve(tokenERC1155.address, ethers.constants.MaxUint256);
    ...
    describe("batch mint function:", async () => {
        it("should revert when caller is not owner: ", async () => {
            await expect(
                tokenERC1155
                    .connect(user1)
                    .batchMint(mkpManager.address, [101, 102, 103], ["this_uri", "this_uri_1", "this_uri_2"])
        ...
        it("should revert when amount length not equal to uris length: ", async () => {
            await expect(
                tokenERC1155.batchMint(mkpManager.address, [10, 11, 12, 13], ["this_uri", "this_uri_1", "this_uri_2"])
            ).to.be.revertedWith("Invalid length");
        });

        it("should revert when receiver address equal to zero address: ", async () => {
            await expect(
                tokenERC1155.batchMint(constants.ZERO_ADDRESS, [10, 11, 12], ["this_uri", "this_uri_1", "this_uri_2"])
            ).to.be.revertedWith("ERROR: invalid address !");
        });

        it("should revert when amount equal to zero address: ", async () => {
            await expect(
                tokenERC1155.batchMint(mkpManager.address, [0, 100, 100], ["this_uri", "this_uri_1", "this_uri_2"])
        ...
        it("should mint success: ", async () => {
            ...
            await tokenERC1155.batchMint(mkpManager.address, [10, 11, 12], ["this_uri", "this_uri_1", "this_uri_2"]);
