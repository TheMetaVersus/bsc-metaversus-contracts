...
describe("Metaversus Manager:", () => {
    ...
    describe("setFee function:", async () => {
        it("should revert when caller is not owner or admin: ", async () => {
            await expect(mtvsManager.connect(user1).setFee(100)).to.be.revertedWith(
                "Ownable: caller is not an owner or admin"
            );
        });
        it("should set treasury success: ", async () => {
            await mtvsManager.setFee(100);
            expect(await mtvsManager.fee()).to.equal(100);

            await mtvsManager.setFee(150);
            expect(await mtvsManager.fee()).to.equal(150);

            await mtvsManager.setFee(200);
            expect(await mtvsManager.fee()).to.equal(200);
        });
    });

    describe("createNFT function:", async () => {
        it("should create success: ", async () => {
            await token.mint(user1.address, "9000000000000000000");
            ...
            await mtvsManager.connect(user1).createNFT(typeNFT, 0);
            expect(await tokenMintERC721.balanceOf(user1.address)).to.equal(1);
            typeNFT = 1; //ERC1155
            await mtvsManager.connect(user1).createNFT(typeNFT, 100);
