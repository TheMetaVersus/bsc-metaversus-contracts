...
describe("NFTMTVSTicket:", () => {
    ...
    describe("setAdmin function:", async () => {
        it("should revert when caller is not owner: ", async () => {
            await expect(nftMTVSTicket.connect(user1).setAdmin(user2.address, true)).to.be.revertedWith(
    ...
    describe("setTreasury function:", async () => {
        it("should revert when caller is not owner: ", async () => {
            await expect(nftMTVSTicket.connect(user1).setTreasury(user2.address)).to.be.revertedWith(
                "Adminable: caller is not an owner or admin"
            );
        });
    ...
    describe("setPrice function:", async () => {
        ...
        it("should revert when price equal to zero: ", async () => {
            await expect(nftMTVSTicket.setPrice(0)).to.be.revertedWith("ERROR: amount must be greater than zero !");
        });
        it("should set treasury success: ", async () => {
    ...
    describe("buy function:", async () => {
        it("should revert when have a NFT: ", async () => {
            ...
            await expect(() => nftMTVSTicket.connect(user1).buy()).to.changeTokenBalance(token, user1, -PRICE);
            ...
            await expect(nftMTVSTicket.connect(user1).buy()).to.be.revertedWith("ERROR: Each account have only one");
        });
    ...
    describe("mint function:", async () => {
        it("should revert when receiver is zero address: ", async () => {
            await expect(nftMTVSTicket.mint(constants.ZERO_ADDRESS)).to.be.revertedWith("ERROR: invalid address !");
        });
        ...
        it("should revert when have a NFT: ", async () => {
            ...
            await expect(nftMTVSTicket.mint(user2.address)).to.be.revertedWith("ERROR: Each account have only one");
        });
